...
export function ResourceCard({ resource, onStatusChange }: ResourceCardProps) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5 flex flex-col justify-between space-y-4 hover:border-slate-300 shadow-sm transition-all">
      {/* Header */}
      ...
      <div className="space-y-2 text-xs">
        <div className="flex items-center justify-between text-slate-700 bg-slate-50 p-2.5 rounded-lg border border-slate-200">
        ...
        {resource.assignedCaseId && (
          <div className="flex items-center justify-between text-xs px-1 text-blue-700">
            <span className="text-slate-500">Assigned to:</span>
            <span className="font-mono font-bold bg-blue-50 px-2 py-0.5 rounded border border-blue-200 text-blue-800">
              {resource.assignedCaseId}
            </span>
          </div>
        )}

        <div className="flex items-center gap-1.5 text-[11px] text-slate-500 px-1 truncate">
